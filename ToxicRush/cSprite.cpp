...
void cSprite::setPixelCollision(SDL_Rect pRect)
{
	SDL_Point sPos = { this->getSpritePos().x,this -> getSpritePos().y };
	for (int x = 0; x < this->getSpriteDimensions().x; x++)
	{
		for (int y = 0; y < this->getSpriteDimensions().y; y++)
		{
			this->pixels[x + y] = { sPos.x ,sPos.y,sPos.x + (x+1), sPos.y + y };
		}
	}
...
vector<SDL_Rect> cSprite::getPixels() 
{
	return pixels;
}

int cSprite::getPixelNumber()
{
	return cSprite::pixels.capacity();
}
